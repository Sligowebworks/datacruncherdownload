...
isnull(v_Expulsions.[Possible Days Attendance],0) as 'possible_days_attendance', 
isnull(v_Expulsions.[Number of Days Expelled],0) as 'number_of_days_expelled', 
(case v_Expulsions.[Total Enrollment Prek-12] when 0 then '--' else cast(isnull(v_Expulsions.[Percent of Days Expelled],0) as char) end) as 'percent_of_days_expelled'

--> changes with each
from v_ExpulsionsDaysLostSchoolDistState v_Expulsions
...
--> changes with each
right(left(fullkey,8),6) = '361914' and (select distinct PartSchIndic from tblAgencyFull agency where  v_Expulsions.year = agency.year and v_Expulsions.agencykey = agency.agencykey) is not null 
))
...
and 
((grade >= (select distinct lowgrade from tblAgencyFull agency where  v_Expulsions.year = agency.year and v_Expulsions.agencykey = agency.agencykey) 
and grade <= (select distinct highgrade from tblAgencyFull agency where v_Expulsions.year = agency.year and v_Expulsions.agencykey = agency.agencykey))
or grade = '99')
