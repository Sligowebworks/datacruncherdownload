...
'total_enrollment_prek-12' = (case when Enrollment is null then '0' when Enrollment < 0 then '*' when Enrollment > 0 then cast(Enrollment as char) else '0' end)
...
'percent_without_disabilities' = (case 
	...
	when Enrollment = 0 then '--' 
		...
		when 0 then 0 
	else (([Non-Disabled]/Enrollment) * 100) 
	end) as char) 
end), 
