...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[procActivitiesExtraCoDataCrunch7]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[procActivitiesExtraCoDataCrunch7]
GO

create procedure procActivitiesExtraCoDataCrunch7
@year char(4),
@cesa char(2)
as
...
isnull(keys.Enrollment,0) AS 'enrollment_grades_6-12', 'pupils_participating' = CASE WHEN keys.suppressed is null THEN '0' 
WHEN keys.suppressed is not null THEN cast(isnull(PupilParticip,0) as char(10)) END, 
...
'participation_rate' = 
(CASE 
	when keys.Enrollment = 0 THEN '--' 
	WHEN keys.suppressed is null THEN '0.0' 
	WHEN keys.suppressed is not null THEN 
	(CASE 
		WHEN PupilParticip is null THEN '0.0' 
		when data.Enrollment = 0 THEN '0.0' 
		WHEN PupilParticip is not null THEN cast(((PupilParticip/data.Enrollment)*100) as varchar(50)) 
	END) 
END)
...
where activities.code in ('at','ac','ms') and v_enrollment.race = 9 and v_enrollment.sex = 9 and v_enrollment.year = @year 
...
(select v_enrollment.year, agency.name, enrollment, suppressed, pupilparticip, code, v_enrollment.fullkey, v_enrollment.gradelabel
from v_enrollment 
...
--where keys.[fullkey] = '013619040075' or keys.[fullkey] = '013619040143'
