...
--> changes with each
right(left(fullkey,8),6) = '361914' and (select distinct PartSchIndic from tblAgencyFull agency where  v_suspensions.year = agency.year and v_suspensions.agencykey = agency.agencykey) is not null 
))
...
and 
((grade >= (select distinct lowgrade from tblAgencyFull agency where  v_suspensions.year = agency.year and v_suspensions.agencykey = agency.agencykey) 
and grade <= (select distinct highgrade from tblAgencyFull agency where v_suspensions.year = agency.year and v_suspensions.agencykey = agency.agencykey))
or grade = '99')
