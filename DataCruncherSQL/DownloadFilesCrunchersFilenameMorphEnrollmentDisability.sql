...
--> changes with each
set @DTSName = 'DataCrunchEnrollmentDisability'
...
INNER JOIN 
--> changes with each
--(SELECT DISTINCT [year] FROM enrollment where fullkey = 'XXXXXXXXXXXX') Years
(SELECT DISTINCT [year] FROM tblPubEnrWWoDisSuppressedSchoolDistStateFlat where fullkey = 'XXXXXXXXXXXX' and [non-disabled] <> 0) Years
