...
--ltrim(category) as 'category', revenue, 'membership' = left(right(prioryear,len(prioryear) - charindex('(',prioryear)),((charindex(' members',right(prioryear,len(prioryear) - charindex('(',prioryear))))-1)), [Revenue Per Member] as 'revenue_per_member', [Percent of Total] as 'percent_of_total'

'cost_type' = (case 
--when ltrim(category) in ('Food and Comm. Serv.','Transportation + Facilities','Current Education Cost', 'Total Cost') then 'Total Cost' 
--when ltrim(category) in ('Transportation','Facilities','Current Education Cost', 'Total Education Cost') then 'Total Education Cost' 
--when ltrim(category) in ('Admin. and Operations','Pupil and Staff Services','Current Education Cost', 'Instruction') then 'Current Education Cost' 
when ltrim(category) in ('Food and Comm. Serv.','Total Cost') then 'Total Cost' 
when ltrim(category) in ('Transportation','Facilities','Transportation + Facilities', 'Total Education Cost') then 'Total Education Cost' 
when ltrim(category) in ('Admin. and Operations','Pupil and Staff Services','Current Education Cost', 'Instruction') then 'Current Education Cost' 
end),

ltrim(category) as 'cost_category', 
'cost_in_dollars' = cost, 
'membership' = left(right(prioryear,len(prioryear) - charindex('(',prioryear)),((charindex(' members',right(prioryear,len(prioryear) - charindex('(',prioryear))))-1)), 
[Cost Per Member] as 'cost_per_member_in_dollars', 
[Percent of Total] as 'percent_of_total_cost'

--> changes with each
from v_expend_2
where year = ? and fullkey <> 'XXXXXXXXXXXX' and CESA = ?
--where year = '1999' and fullkey <> 'XXXXXXXXXXXX' and CESA = '01' 
--and right(left(fullkey,8),2) <> '14' 

and (right(left(fullkey,8),2) <> '14' or (
--> changes with each
right(left(fullkey,8),6) = '361914' and (select distinct PartSchIndic from agency where  v_expend_2.year = agency.year and v_expend_2.agencykey = agency.agencykey) is not null 
))
