...
[Expulsion Type] as 'expulsion_type', [Total # Expelled] as 'total_number_expelled', [# One Year or Less] as 'number_one_year_or_less', 
[# More Than One Year] as 'number_more_than_one_year', [# Permanent] as 'number_permanent'

--> changes with each
from vexpulsionservices 
...
--> changes with each
right(left(fullkey,8),6) = '361914' and (select distinct PartSchIndic from agency where  vexpulsionservices.year = agency.year and vexpulsionservices.agencykey = agency.agencykey) is not null 
))
...
order by [District Number],[School Number], expulsion_type
