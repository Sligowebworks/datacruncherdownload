...
/****** Object:  View dbo.v_EnrollFlatRaceSexGradesExSS3    Script Date: 1/19/2007 2:59:09 PM ******/
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[v_EnrollFlatRaceSexGradesExSS3]') and OBJECTPROPERTY(id, N'IsView') = 1)
drop view [dbo].[v_EnrollFlatRaceSexGradesExSS3]
GO



CREATE view dbo.v_EnrollFlatRaceSexGradesExSS3 as
...
'Enrollment' = 
sum(
--case when right(d.fullkey,1) = 'X' and left(d.fullkey,1) <> 'X' then 
---- must leave out 8's in race to get number that match current pages
--(CASE WHEN d.gradecode in (12,16,20,24,28,32,36,40,44,48,52,56,60,64,70,71,72,73,99) AND  d.race in (1,2,3,4,5,8) and d.sex = 9 THEN isnull(d.enrollment,'0') ELSE 0 END)
--else 
--(CASE WHEN d.gradecode in (12,16,20,24,28,32,36,40,44,48,52,56,60,64,70,71,72,73,99) AND  d.race = 9 and d.sex <> 9 THEN isnull(d.enrollment,'0') ELSE 0 END)
--(CASE WHEN d.gradecode = 99 AND ((d.race = 9 and d.sex <> 9) or (d.race <> 9 and d.sex = 9)) THEN isnull(d.enrollment,'0') ELSE 0 END)
--(CASE WHEN d.gradecode in (12,16,20,24,28,32,36,40,44,48,52,56,60,64,70,71,72,73,99) AND ((d.race = 9 and d.sex <> 9) or (d.race <> 9 and d.sex = 9)) THEN isnull(d.enrollment,'0') ELSE 0 END)
(CASE WHEN d.gradecode in (12,16,20,24,28,32,36,40,44,48,52,56,60,64,70,71,72,73,99) AND (d.race = 9 and d.sex = 9) THEN isnull(d.enrollment,'0') ELSE 0 END)
--(CASE WHEN d.gradecode in (12,16,20,24,28,32,36,40,44,48,52,56,60,64,70,71,72,73,99) AND  d.racecode = 9 and d.sexcode = 9 THEN isnull(d.enrollment,'0') ELSE 0 END)
--end
)
,
...
case 
	...
	then cast(min(cast(isnull(d.suppressed,0) as int)) as char) -- don't sum the suppression indicator, just return the lowest suppression indicator as char
		...
		--(CASE WHEN d.gradecode in (12,16,20,24,28,32,36,40,44,48,52,56,60,64,70,71,72,73) AND  d.race <> 9 and d.sex <> 9 THEN cast(isnull(d.Suppressed,'0') as int) 	ELSE 0 END) 
		(CASE WHEN d.gradecode in (12,16,20,24,28,32,36,40,44,48,52,56,60,64,70,71,72,73,99) AND (d.race = 9 and d.sex = 9) THEN isnull(d.enrollment,'0') ELSE 0 END)
	as char)) -- otherwise apply the same formula as to enrollment
end,
...
--year = '2006' and left(right(fullkey,10),4) = '0280' and 
--fullkey = 'XXXXXXXXXXXX' and

-- must include below
cast(year as int) > 1996 
and d.gradecode in (12,16,20,24,28,32,36,40,44,48,52,56,60,64,70,71,72,73) 
--and d.gradecode = 99
...
GRANT  SELECT  ON [dbo].[v_EnrollFlatRaceSexGradesExSS3]  TO [public]
GO
