...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[procActivitiesCommunityDataCrunch3_old2]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[procActivitiesCommunityDataCrunch3_old2]
GO

create procedure procActivitiesCommunityDataCrunch3_old2 
@year char(4),
@cesa char(2)
as
...
isnull(keys.Enrollment,0) AS 'enrollment_grades_9-12', 'pupils_participating' = CASE WHEN keys.suppressed is null THEN '0' 
...
'participation_rate' = CASE when keys.Enrollment = 0 THEN '--' WHEN keys.suppressed is null THEN '0.0' WHEN keys.suppressed is not null THEN CASE WHEN PupilParticip is null THEN '0.0' 
when data.Enrollment = 0 THEN '0.0' WHEN PupilParticip is not null THEN cast(((PupilParticip/data.Enrollment)*100) as varchar(50)) END END
