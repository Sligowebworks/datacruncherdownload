...
-- codes in some legacy columns are doubled - must support 
drop FUNCTION dbo.ConvertFloatToCharAndTrapNumericCodesDoubledCodes
go

CREATE FUNCTION dbo.ConvertFloatToCharAndTrapNumericCodesDoubledCodes (@IncomingCode float)
RETURNS char(8)
AS
BEGIN 
	declare @Outgoingcode char(8)
	...
	--if isnumeric(rtrim(@IncomingCode)) = 0 set @Outgoingcode = 'ERROR - NOT NUMERIC'
	if rtrim(@IncomingCode) = '-10' set @Outgoingcode = '*'
	if rtrim(@IncomingCode) = '-20' set @Outgoingcode = '*'
	if rtrim(@IncomingCode) = '-40' set @Outgoingcode = 'N/A'
	--else set @Outgoingcode = 'xxx'
	--set @Outgoingcode = @IncomingCode 
	RETURN rtrim(ltrim(@Outgoingcode))
