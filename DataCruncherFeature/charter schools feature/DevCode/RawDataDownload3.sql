...
'race_ethnicity' = 
(case 
when Groupnum IN ('14','15','16','17','18','19','20')
then rtrim([GroupName])
else 'All Races'
end), 

'gender' = 
(case 
when Groupnum IN ('11','12', '13')
then rtrim([GroupName])
else 'Both Genders'
end), 
...
'disability_status' = 
(case 
when Groupnum IN ('25','26')
then rtrim([GroupName])
else 'Students with and without Disabilities'
end), 

'economic_status' = 
(case 
when Groupnum IN ('27','28')
then rtrim([GroupName])
else 
...
(case 
when Groupnum IN ('21','22')
then rtrim([GroupName])
else 'Limited English Proficient and English Proficient'
end), 

'migrant_status' = 
(case 
when Groupnum IN ('23','24')
then rtrim([GroupName])
else 'Both Genders'
end), 

'year' = v_WSAS.[Year], 
'tblAgency_key' = v_WSAS.[AgencyKEY], 
v_WSAS.[CESA], 
...
'school_type' = (case right(v_WSAS.fullkey,1) when 'X' then 'Summary' else 
...
left outer join tblAgency on v_WSAS.year = tblAgency.year and v_WSAS.fullkey = tblAgency.fullkey
...
--and v_WSAS.Fullkey = '01361903XXXX' 
and v_WSAS.Fullkey = '013619040022' 
--and Groupnum = '7' 
